...
Entering the devshell installs the Git hooks, so a clone needs no separate step. Their entries name `bun` by store path and then a `package.json` script, so a commit made outside the devshell runs the same tools, and `bun.lock` stays the only place a tool version is decided. Formatting and types run on commit, and `bun run ci` on push. `.pre-commit-config.yaml` is generated by the flake and is not tracked.
