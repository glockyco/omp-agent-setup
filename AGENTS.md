...
Source-of-truth for the user's global OMP (Oh My Pi) agent setup. Deploys managed files to `~/.omp/agent/`, owns the plugin-fork manifest, ships a small Bun CLI. The deployed-global guidance lives in [`agent/AGENTS.md`](./agent/AGENTS.md).
...
| `bun run update-{superpowers,plannotator}` | Rebase the fork's `omp-local` onto `upstream/main`; print the new SHA to record. |
...
TypeScript strict. Biome enforces format and lint; `bun run fix` rewrites.
...
| Add relative imports outside `extensions/` to `superpowers-bootstrap.ts` | Inline the helper. The file is symlinked; relative imports resolve against the symlink path and break the loader (commit `c313a49`). |
